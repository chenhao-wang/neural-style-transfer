...
CONTENT_IMAGE = 'images/wuhan.jpg'
...
BETA = 100
# Path to the deep learning model. This is more than 500MB
...
def generate_noise_image(content_image, noise_ratio=NOISE_RATIO):
    ...
    """
    # generate noise image as the same size of content image
    noise_image = tf.Variable(
        tf.random_normal([1, content_image.shape[1], content_image.shape[2], COLOR_CHANNELS]))
...
def load_vgg_model(path, shape):
    ...
    graph = {}
    graph['input'] = tf.Variable(np.zeros((1, shape['h'], shape['w'], COLOR_CHANNELS)), dtype='float32')
...
if __name__ == '__main__':
    with tf.Session() as sess:
        ...
        shape = {'h': content_image.shape[1], 'w': content_image.shape[2]}
        # resize style image to the same shape
        style_image = load_image(STYLE_IMAGE)
        style_image = cv2.resize(style_image[0], dsize=(shape['w'], shape['h']), interpolation=cv2.INTER_CUBIC)
        style_image = np.reshape(style_image, ((1,) + style_image.shape))

        # Load the model.
        model = load_vgg_model(VGG_MODEL, shape)
        ...
        for it in range(ITERATIONS):
            ...
            if it % 100 == 0:
                ...
                J, Jc, Js = sess.run([total_loss, content_loss, style_loss])
